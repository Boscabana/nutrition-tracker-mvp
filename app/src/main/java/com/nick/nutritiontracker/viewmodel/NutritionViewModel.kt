...
class NutritionViewModel(application: Application) : AndroidViewModel(application) {
                ...
                if (autoAddToShoppingList) {
                    meal.ingredients.forEach { ing ->
                        ...
                        val item = ShoppingItem(
                            ...
                            unit = ing.unitLabel,
                            isAutoGenerated = true,
                            category = foodItem?.category ?: (foodItem?.parentId?.let { pId -> foods.find { it.id == pId }?.category }),
                            householdId = householdId,
    ...
    fun updateFood(updatedFood: FoodItemEntity) {
        ...
        if (index != -1) {
            ...
            // 1. Sync with LOCAL entries (Diary - past data)
            // User specifically said NOT to refresh past diary entries to preserve history.
            // However, we still sync name/kcal if they changed, as per existing logic.
            // But let's be careful. Existing code already does this loop:
            ...
            // 3. Sync with HOUSEHOLD data (Shopping List & Planner)
            val householdId = firebaseManager.household.value?.id
            if (householdId != null) {
                viewModelScope.launch {
                    // Update active shopping items
                    shoppingList.filter { 
                        !it.isChecked && it.name.equals(finalFood.name, ignoreCase = true) 
                    }.forEach { item ->
                        val updatedItem = item.copy(
                            category = finalFood.category,
                            isPantryItem = finalFood.isPantryItem
                        )
                        firestoreRepository.updateShoppingItem(householdId, updatedItem)
                    }

                    // Update planned entries
                    plannedEntries.filter { 
                        it.foodItemId == finalFood.id || (it.isMeal && it.mealIngredients?.any { ing -> ing.foodItemId == finalFood.id } == true)
                    }.forEach { entry ->
                        if (entry.isMeal) {
                            ...
                            firestoreRepository.addPlannedEntry(householdId, entry.copy(
                                name = finalFood.name,
                                brand = finalFood.brand,
                                kcalPer100g = finalFood.kcalPer100g,
                                proteinPer100g = finalFood.proteinPer100g,
