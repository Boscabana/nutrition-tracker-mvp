...
data class FoodEntryEntity(
    ...
    val unit: String,
    val grams: Double,
    // Snapshot of the food item data
    val foodItemId: Long,
    val name: String,
    ...
    fun displayAmount(): String = if (unit == "portion") {
        "${clean(amount)} ${defaultPortionName ?: "Portion"}"
    } else {
        "${clean(grams)} g"
    }

    private fun clean(v: Double): String = if (v % 1.0 == 0.0) "%.0f".format(v) else "%.1f".format(v)
}
